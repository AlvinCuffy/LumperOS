...
export default function JobDetailModal({ job, onClose, onDelete }) {
  ...
  const hasExtras = job.additives?.length > 0;
      ...
      <div className="modal-sheet">
        ...
        <div className="rounded-2xl p-4 mb-4 border border-white/5" style={{ background: 'rgba(74,222,128,0.06)' }}>
          <div className="text-xs text-slate-500 uppercase tracking-widest mb-1">
            {isPaid ? 'Total Paid' : 'Base Pay'}
          </div>
          <div className="font-headline text-4xl font-black text-emerald-400">${fmt(job.basePay)}</div>
          {!isPaid && hasExtras && (
            <div className="text-xs text-emerald-400/50 mt-1.5">+ additives · confirmed on pay stub</div>
          )}
        </div>
        ...
        {hasExtras && (
          <div className="mb-5">
            <div className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-2">
              {isPaid ? 'Additives & Extras' : 'Logged Additives'}
              {!isPaid && (
                <span className="text-yellow-400/60 normal-case text-[9px]">· pay amounts confirmed on stub</span>
              )}
            </div>
            <div className="flex flex-wrap gap-1.5">
              {job.additives.map(a => (
                <span key={a} className={`tag tag-teal${!isPaid ? ' opacity-60' : ''}`}>{a}</span>
              ))}
            </div>
          </div>
        )}
